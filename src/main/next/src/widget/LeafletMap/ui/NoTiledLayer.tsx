...
export const NonTiledLayer = Layer.extend({
  emptyImageUrl:
    "data:image/gif;base64,R0lGODlhAQABAHAAACH5BAUAAAAALAAAAAABAAEAAAICRAEAOw==", // 1px transparent GIF
  ...
  _initCanvas: function initCanvas() {
    const canvas = DomUtil.create(
      "canvas",
      "leaflet-image-layer"
    ) as HTMLCanvasElement & { _image: HTMLImageElement }
...
;(NonTiledLayer as any).WMS = NonTiledLayer.extend({
    ...
    for (i in options) {
      if (
        !Object.prototype.hasOwnProperty.call(
          NonTiledLayer.prototype.options,
          i
        ) &&
        !(
          Layer &&
          Object.prototype.hasOwnProperty.call(
            (Layer.prototype as any).options,
            i
          )
        )
      ) {
  ...
  getImageUrl: function getImageUrl(bounds, width, height) {
    ...
    const bbox = (
      this._wmsVersion >= 1.3 && this._crs === CRS.EPSG4326
        ? [se.y, nw.x, nw.y, se.x]
        : [nw.x, se.y, se.x, nw.y]
    ).join(",")
...
function createInfoWMSNoTiledLayer(props, context) {
  var instance = new NonTiledLayer.WMS(props.url, props.params);
  return {
      instance: instance,
      context: context,
  };
}
function updateInfoWMSNoTiledLayer(layer, props, prevProps) {
  updateGridLayer(layer, props, prevProps);
}
var InfoWMSNoTiledLayer = createTileLayerComponent(createInfoWMSNoTiledLayer, updateInfoWMSNoTiledLayer);
export { InfoWMSNoTiledLayer };
